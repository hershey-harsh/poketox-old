...
class Collectors(commands.Cog):
    ...
    async def view(self, ctx, *, member: discord.Member = None):
        ...
        pages = ViewMenuPages(
            source=AsyncEmbedListPageSource(
                self.doc_to_species_emoji(result or {}),
                title=str(member),
                format_item=x,
            )
        )
    ...
    @commands.max_concurrency(1, commands.BucketType.user)
    @commands.cooldown(1, 3, commands.BucketType.user)
    async def forceremove(self, ctx, *, user: FetchUserConverter, select: Literal['Shiny Hunt', 'Collect List', 'Regional List', 'Regional Forms'] = None):
